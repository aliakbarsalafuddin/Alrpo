...
class Apple(GameObject):
    def __init__(self, frame_size_x, frame_size_y):
        ...
        super().__init__(pygame.Color(255, 0, 0), position)
...
class Game:
    def __init__(self):
        ...
        self.apple_purpel = Apple(self.frame_size_x, self.frame_size_y)
        self.apple_purpel.color = pygame.Color(128, 0, 128)  # Set warna apel ungu
        self.score = 0
    ...
    def run(self):
        while True:
            ...
            # Cek apakah ular makan apel merah
            if self.snake.position == self.apple.position:
                self.score += 1
                self.apple.respawn(self.frame_size_x, self.frame_size_y)
            # Cek apakah ular makan apel ungu
            elif self.snake.position == self.apple_purpel.position:
                self.score += 1
                self.apple_purpel.respawn(self.frame_size_x, self.frame_size_y)
            else:
                self.snake.shrink()
            ...
            self.apple_purpel.draw(self.game_window)
            self.show_screen()
